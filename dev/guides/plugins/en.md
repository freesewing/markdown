...
const Pattern = new freesewing.Design(config, bundlePlugin)
```
...
const Pattern = new freesewing.Design(config, [bundlePlugin, gorePlugin] )
```
...
Run-time plugin are loaded at run time, by passing them to the `use` methof of
